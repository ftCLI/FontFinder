...
class FontFinder:
    """
    A class that finds fonts in a given path. It can search for fonts in a directory and its subdirectories, and can
    also handle a single font file.
    ...
    The class returns a list of TTFont objects, or of TTFont subclass objects, that meet the specified criteria.

    Attributes:

        input_path: A Path object pointing to the input path. The input path can be a directory or a file.
        ...
        recalc_timestamp: A boolean indicating whether to recalculate the modified timestamp on save.
        recalc_bboxes: A boolean indicating whether to recalculate the bounding boxes on save.
        lazy: A boolean indicating whether to load the font lazily. If lazy is set to True, many data structures are
            loaded lazily, upon access only. If it is set to False, many data structures are loaded immediately.
        ...
        for file in files:
            try:
                font = self.return_cls(
                    file, lazy=self.lazy, recalcTimestamp=self.recalc_timestamp, recalcBBoxes=self.recalc_bboxes
                )
